...
def test_export_torch_model_to_onnx():
    ...
    if os.path.exists(file_path):
        os.remove(file_path)
    model_io.export_torch_model_to_onnx(net, dummy_input, "model.onnx")
...
def test_export_to_torchscript():
    ...
    model_io.export_to_torchscript(model, path_and_filename)
    assert Path(path_and_filename).exists()


def test_export_to_pickle():
    model = Net()
    path_and_filename = settings.BASE_DIR.parent / "resources" / "tests" / "model.pkl"
    model_io.export_to_pickle(model, path_and_filename)
    assert Path(path_and_filename).exists()


def test_load_model_by_struct():
    path_and_filename = settings.BASE_DIR.parent / "resources" / "tests" / "model.pth"
    model = model_io.load_model_by_struct(Net, path_and_filename)
...
# def test_load_model_by_pickle():
#     path_and_filename = settings.BASE_DIR.parent / "resources" / "tests" / "model.pkl"
#     model = model_io.load_model_by_pickle(path_and_filename)
#     # 读取数据集
#     default_dataset_dir = settings.BASE_DIR / "data" / "default_dataset"
#     dataset = MNIST(default_dataset_dir, download=True, transform=transforms.ToTensor())
#     for x, y in dataset:
...
#     path_and_filename = settings.BASE_DIR.parent / "resources" / "tests" / "model.pt"
#     model = model_io.load_model_by_torchscript(path_and_filename)
#     # 读取数据集
#     default_dataset_dir = settings.BASE_DIR / "data" / "default_dataset"
#     dataset = MNIST(default_dataset_dir, download=True, transform=transforms.ToTensor())
#     for x, y in dataset:
